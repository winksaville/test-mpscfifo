...
MpscFifo_t *initMpscFifo(MpscFifo_t *pQ, Msg_t *pStub) {
  DPF("initMpscFifo: pQ=%p stub=%p stub->pPool=%p\n",
      pQ, pStub, pStub->pPool);
  pStub->pNext = NULL;
  pQ->pHead = pStub;
  pQ->pTail = pStub;
  return pQ;
...
Msg_t *deinitMpscFifo(MpscFifo_t *pQ) {
  ...
  if (pStub->pPool == NULL) {
    ...
    DPF("deinitMpscFifo:+pQ=%p ret stub=%p stub->pPool=%p\n", pQ, pStub, pStub->pPool);
    ret(pStub);
    DPF("deinitMpscFifo:-pQ=%p ret stub=%p stub->pPool=%p\n", pQ, pStub, pStub->pPool);
    return NULL;
  }
...
void add(MpscFifo_t *pQ, Msg_t *pMsg) {
  DPF("add:+pQ=%p msg=%p arg1=%lu arg2=%lu\n",
        pQ, pMsg, pMsg->arg1, pMsg->arg2);
  pMsg->pNext = NULL;
...
Msg_t *rmv_non_stalling(MpscFifo_t *pQ) {
  ...
  if (pNext != NULL) {
    ...
    pQ->pTail = pNext;
    DPF("rmv_non_stailling: got msg pQ=%p msg=%p arg1=%lu arg2=%lu\n",
        pQ, pTail, pTail->arg1, pTail->arg2);
    return pTail;
  } else {
    DPF("rmv_non_stalling: 'empty' pQ=%p msg=NULL\n", pQ);
    return NULL;
  }
}
...
Msg_t *rmv(MpscFifo_t *pQ) {
  ...
  if ((pNext == NULL) && (pTail == pQ->pHead)) {
    // Q is empty
    DPF("rmv: empty pQ=%p pNext=%p pTail=%p == pHead=%p\n",
        pQ, pNext, pTail, pQ->pHead);
    return NULL;
  } else {
    if (pNext == NULL) {
      // Q is NOT empty but producer was preempted at the critical spot
      DPF("rmv: stalling pQ=%p pNext=%p pTail=%p != pHead=%p\n",
          pQ, pNext, pTail, pQ->pHead);
      uint32_t i;
    ...
    pTail->arg2 = pNext->arg2;
    pQ->pTail = pNext;
    DPF("rmv: got msg pQ=%p msg=%p arg1=%lu arg2=%lu\n",
        pQ, pTail, pTail->arg1, pTail->arg2);
    return pTail;
  }
}
...
void ret(Msg_t* pMsg) {
  if ((pMsg != NULL) && (pMsg->pPool != NULL)) {
    DPF("ret: pool=%p msg=%p arg1=%lu arg2=%lu\n",
        pMsg->pPool, pMsg, pMsg->arg1, pMsg->arg2);
    add(pMsg->pPool, pMsg);
  } else {
    if (pMsg == NULL) {
      DPF("ret: No msg msg=%p\n", pMsg);
    } else {
      DPF("ret: No pool pool=%p msg=%p arg1=%lu arg2=%lu\n",
          pMsg->pPool, pMsg, pMsg->arg1, pMsg->arg2);
    }
  }
}
...
void send_rsp_or_ret(Msg_t* msg, uint64_t arg1) {
  if (msg->pRspQ != NULL) {
    ...
    msg->pRspQ = NULL;
    msg->arg1 = arg1;
    DPF("send_rsp_or_ret: send pRspQ=%p rsp arg1=%lu arg2=%lu\n",
        pRspQ, msg->arg1, msg->arg2);
    add(pRspQ, msg);
  } else {
    DPF("send_rsp_or_ret: ret msg=%p arg1=%lu arg2=%lu\n",
        msg, msg->arg1, msg->arg2);
    ret(msg);
  }
}
